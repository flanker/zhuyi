...
!function( $ ) {
	...
	Datepicker.prototype = {
		...
		update: function(){
			var dateString = this.isInput ? this.element.prop('value') : this.element.data('date') || this.element.find('input').prop('value');
			if (dateString === '') {
				this.date = new Date();
			} else {
				this.date = DPGlobal.parseDate(dateString, this.format, this.language);
			}
